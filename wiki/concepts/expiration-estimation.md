...
2. **Tag iteration**: Each tag in the `category_tags` list is normalized: lowercased, stripped, split on `:` keeping the last segment (this removes language prefixes such as `en:`), then mapped through `OFF_TO_INTERNAL` first and `CATEGORY_ALIASES` second — the same path as `normalize_category` in `backend/config.py` (e.g. `pastas` → `pasta`, `pet-food` → `animali`, `yogurt` → `yogurts`, `milk` → `fresh-milk`). The normalized string is then checked against every key in `DEFAULT_SHELF_LIFE` (except `"default"`) using **exact equality**. On the iOS side, these category keys correspond to the server-driven options in [CategoryPicker](../components/ios-category-picker.md) (see [Category Registry](./category-registry.md)).
...
Defined in [backend/config.py](../config/backend-config.md): 27 categories plus the `"default"` fallback (extended to align with the OFF taxonomy — see [Backend Service — Expiration](../modules/backend-service-expiration.md) for the full table). The `"default"` key serves as the fallback when no category matches. Singular/plural variants are covered by `CATEGORY_ALIASES`, while broader OFF plural/synonym variants are covered by `OFF_TO_INTERNAL`, applied first.

## Matching Logic Details

Matching is **exact and case-insensitive** on the normalized tag: a tag like `"en:dairy-products"` normalizes to `"dairy-products"`, which equals no key and falls back to 30 days. Aliases rescue common variants (`"en:cheese"` → `"cheeses"` → 30 days; `"en:yogurt"` → `"yogurts"` → 14 days), and `OFF_TO_INTERNAL` rescues broader OFF variants before aliases are consulted (`"en:pastas"` → `"pasta"` → 365 days instead of the 30-day fallback; `"en:pet-food"` → `"animali"` → 365 days). Unknown tags can never accidentally match — adding new keys to `DEFAULT_SHELF_LIFE` is safe by construction (unlike the old substring rule).
...
- Otherwise `category` + `off_category_tags` are merged, normalized (lower/strip/split/`OFF_TO_INTERNAL`/`CATEGORY_ALIASES`/dedup — same path as `normalize_category`), and estimated → `(estimated, True)`.
...
- Exact matching means new `DEFAULT_SHELF_LIFE` keys can be added without risk of accidental matches; variants should be covered via `OFF_TO_INTERNAL` (broad OFF plurals/synonyms) or `CATEGORY_ALIASES` (legacy singular/plural) instead.
