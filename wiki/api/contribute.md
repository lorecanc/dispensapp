...
Both endpoints are disabled by default behind `OFF_WRITE_ENABLED` and share an in-memory per-IP rate limit (10 req/min, 60 s window). Staging write base URL defaults to `https://world.openfoodfacts.net/cgi`; prod `.org` only via env override.
...
**Response**: `ContributeResponse` (`200`): `{"ok": true, "code": "<barcode>", "message": "Contributo inviato a Open Food Facts"}`
...
Oversize fails fast on declared `Content-Length` (5 MB + 1 KB multipart tolerance) then on actual bytes (413). Declared `Content-Type` must match magic-byte detection. HEIC skips dimension check.
...
**Source**: `backend/services/off.py:72-135`, `backend/services/off.py:138-178`
...
- Rate limit: `_check_rate_limit(ip)`, shared bucket across both endpoints, 10 req/min per IP (`backend/routes/contribute.py:24-36`).
...
| Status | When | Detail |
|--------|------|--------|
| 400 | `consent_cc_bysa=false` | `Consenso CC BY-SA obbligatorio per contribuire a Open Food Facts` |
| 403 | `OFF_WRITE_ENABLED=false` | `Contribuzione a Open Food Facts disabilitata sul server` |
...
| 502 | OFF transport error or `status != 1` refusal | `Errore durante la comunicazione con Open Food Facts` / `Open Food Facts ha rifiutato il contributo` |

**Prod note**: in-memory limiter is a no-dependency mitigation; replace with `slowapi`/Redis-backed auth before go-live (`TODO(prod)` in `backend/routes/contribute.py:23`).
