def test_inventory_create_quantity_zero_422(client):
    resp = client.post(
        "/api/inventory",
...
def test_inventory_create_quantity_1000_422(client):
    resp = client.post(
        "/api/inventory",
...
def test_inventory_create_quantity_negative_422(client):
    resp = client.post(
        "/api/inventory",
...
def test_inventory_manual_quantity_out_of_range_422(client):
    for q in [0, 1000, -1]:
        resp = client.post("/api/inventory/manual", json={"name": "Latte", "quantity": q})
...
def test_barcode_invalid_inventory_create_422(client):
    resp = client.post(
        "/api/inventory",
...
def test_name_empty_422_inventory(client):
    resp = client.post(
        "/api/inventory",
...
def test_name_empty_422_manual(client):
    resp = client.post("/api/inventory/manual", json={"name": "", "quantity": 1})
    assert resp.status_code == 422
    resp2 = client.post("/api/inventory/manual", json={"name": "   ", "quantity": 1})
    assert resp2.status_code == 422


def test_name_missing_422(client):
    resp = client.post("/api/inventory/manual", json={"quantity": 1})
    assert resp.status_code == 422
